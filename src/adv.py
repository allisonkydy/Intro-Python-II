...
room = {
    'shack':  Room(wrapper.fill("Shack"),
                   wrapper.fill("The thin wooden walls are rotting and the floor is nothing but dirt. It smells pretty bad in here. Light filters in from the door to the south."),
                   True),

    'garden': Room(wrapper.fill("Overgrown Garden"), wrapper.fill("It used to be a garden, but weeds have sprung up between the neatly planted rows. The air carries a light floral fragrance. You hear the rustling of chickens to the west. An iron gate guards the path east. To the south is a large forest. The small shack is just north."),
                      True),

    'coop':   Room(wrapper.fill("Chicken Coop"), wrapper.fill("Cozy stalls hold about twenty dozing chickens. They look well-fed and content. East is the door to the garden."),
                 True),

    'ruins':  Room(wrapper.fill("Ancient Ruins"), wrapper.fill("Smooth, mossy rocks are scattered about. Some are still loosely connected, forming the remains of a circular tower."),
                    True),

    'forest': Room(wrapper.fill("Forest"), wrapper.fill("Trees tower over your head and the floor is thick with underbrush. The canopy blocks out all light from above. You try not to look directly at the shadows. A faint bubbling sound comes from the south. A small dirt path leads west. The garden lies north."),
                   False),

    'glen':   Room(wrapper.fill("Secluded Glen"), wrapper.fill("Mossy rocks slope gently down to a narrow stream. The air is cool and damp. The path bends from east to south."),
                   True),

    'river':  Room(wrapper.fill("River"), wrapper.fill("The river bubbles excitedly, flowing from east to west. A small cluster of lily pads clings to rocks in the shallows. The forest is back north."),
                    True),

    'cliff':  Room(wrapper.fill("Tall Cliff"), wrapper.fill("Water cascades down from the cliff with a dull roar. You squint your eyes against the spray. The path stretches to the north and south."),
                        True),

    'cave':   Room(wrapper.fill("Secret Cave"), wrapper.fill("Aha, a hidden cave! The walls seem to glow with a faint light. Water drops gently from cracks in the ceiling. The entrance is to the north."),
                   True),
}
...
item = {
    'plank': Item('plank', "A plank of soft wood, perfect for carving", True, "A fresh plank of wood leans against the far wall."),
...
room['shack'].add_item(item['plank'])
...
class Actions:
    ...
    def flute_beaver(self, flute, beaver):
        ...
        print(wrapper.fill("The beaver is drawn to the sound of the flute. It looks friendly now."))
    ...
    def beaver_river(self, beaver, river):
        ...
        room['cliff'].description = "The cliff towers above you, silent and serene. A small trickle of water runs down its face. There's a small opening in the rock where the waterfall once was. The path stretches to the north and south."
        river.description = "Shallow and rocky. It's blocked by the beavers' dam."
        river.location_desc = "The river no longer flows."
        room['river'].description = "A large beaver dam spans the river, stopping the flow. A small cluster of lily pads clings to rocks in the shallows."
        room['forest'] = "Trees tower over your head and the floor is thick with underbrush. The canopy blocks out all light from above. You try not to look directly at the shadows. A faint chattering sound comes from the south. A small dirt path leads west. The garden lies north."
    ...
    while True:
        ...
        if input_length == 1:
            ...
            # print list of items in room if player looks around ('l' or 'look')
                ...
                if player.is_player_or_room_lit():
                    ...
                    print(wrapper.fill("It's too dark to see anything."))
            ...
            # print error message if user enters invalid input
                ...
                print(wrapper.fill("Input not valid, please try again"))
        ...
        # if the form of the input is 'verb object'
            ...
            if not object_name in item:
                print(wrapper.fill("That item does not exist"))
            ...
            # error message for improper usage of 'use'
                ...
                print(wrapper.fill("What should I use this on?"))
        ...
        # if the form of the input is 'use [item] on [item]'
            ...
            if item_used in item and item_target in item:
                ...
                print(wrapper.fill("Whoops, that won't work"))
